...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const stopTurn = useCallback(() => {
    const sid =
      ...
      resumeParam;
    if (!sid) {
      setBanner(t.chat.stopNoSession ?? "No active session to stop.");
      return;
    }
    completionGw
      .request<{ interrupted?: boolean }>("session.interrupt", { session_id: sid })
      .catch((e: Error) => {
        // Surface real failures (e.g. compute-host 5019) instead of a silent
        // no-op — the user pressed Stop and deserves to know it didn't land.
        setBanner(`${t.chat.stopFailed ?? "Stop failed"}: ${e?.message ?? "unknown"}`);
      });
            ...
            >
              ...
              >
                ...
                {/* Full status strip — on phones this wraps to its own second
                    row so every segment (model/cwd/subagent/bg/duration) stays
                    visible; on desktop it sits inline next to the usage bar. */}
                <SessionStatusBar
